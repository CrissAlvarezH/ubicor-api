...
def upload_unicor_imgs(domain, user, password):
    ...
    # get token
    response = requests.post(f"{BASE_URL}/login", data={"username": user, "password": password})
    ...
    for building in response.json():
        ...
        path = os.path.join(os.path.dirname(__file__), "ubicor_imgs", building.get("code"))
        if not os.path.exists(path):
            print("does not exists", building.get("code"), building.get("name"))
            continue

        print("------ upload imgs from building:", building.get("code"), " -------")
